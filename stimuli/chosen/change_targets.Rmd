...
dat <- read.csv("nicenboim_stone_cloze_MANUALLY_REPLACED_TARGETS.csv", header=TRUE)
```
...
```{r merge-keep-df}
# Replace the targets
keep_changed <- rbind(keep, fixed)
keep_changed <- keep_changed %>% 
  mutate(noun = if_else(noun_new != "", noun_new, noun))
keep_changed <- keep_changed %>% 
  mutate(clozenoun = if_else(clozenoun_new != "", clozenoun_new, clozenoun))
...
unique(sort(duplicates_stems$clozenoun))
